...
@Injectable()
export class DouyinCheckerService {
  private readonly logger = new Logger(DouyinCheckerService.name);
  ...
  async checkDouyinUserVideos(secUserId: string): Promise<any> {
    this.logger.log(`Checking videos for Douyin user: ${secUserId}`);
    ...
    // 注意：抖音可能需要特定的User-Agent或其他Header来模拟浏览器请求
    // 否则可能会被识别为爬虫而返回错误或空数据
    const headers = {
      'User-Agent':
      ...
      // TODO: Securely manage cookies. Avoid hardcoding sensitive information.
      // Cookie: 'your_douyin_cookie_here', // 如果需要登录才能访问，可能需要Cookie
    };
    ...
    try {
      ...
      if (cachedData && cachedData.aweme_id && cachedData.create_time) {
        ...
        this.logger.log(`Cached data for ${secUserId}: aweme_id=${cachedLatestAwemeId}, create_time=${cachedLatestCreateTime}`);
      }
    } catch (redisError) {
      this.logger.error(`Failed to retrieve cached data for ${secUserId}: ${redisError.message}`);
    }
    ...
    try {
      ...
      this.logger.log(`Successfully fetched data for ${secUserId}`);

      const awemeList = response.data?.aweme_list;

      if (!awemeList || awemeList.length === 0) {
        this.logger.log(`No videos found for ${secUserId}.`);
        return null;
      }

      // Filter out pinned videos (is_top === 1)
      const nonTopVideos = awemeList.filter((video: any) => video.is_top !== 1);

      if (nonTopVideos.length === 0) {
        this.logger.log(`No non-top videos found for ${secUserId}.`);
        return null;
      }

      const latestVideo = nonTopVideos[0]; // Get the first non-top video
      const currentAwemeId = latestVideo.aweme_id;
      const currentCreateTime = latestVideo.create_time; // Douyin API returns timestamp in seconds

      let isNewVideo = false;
      if (cachedLatestAwemeId === null) {
        // No cached data, this is the first time we're checking
        isNewVideo = true;
        this.logger.log(`First check for ${secUserId}. Storing latest video.`);
      } else if (
        currentAwemeId !== cachedLatestAwemeId &&
        currentCreateTime > cachedLatestCreateTime
      ) {
        isNewVideo = true;
        this.logger.log(`New video detected for ${secUserId}!`);
      } else {
        this.logger.log(`No new video for ${secUserId}.`);
      }

      if (isNewVideo) {
        // Update cache with the new latest video
        ...
        if (videoUrl) {
          this.logger.log(`New video URL: ${videoUrl}`);
          return {
            aweme_id: currentAwemeId,
            create_time: currentCreateTime,
            video_url: videoUrl,
            title: latestVideo.desc, // Video description as title
            digg_count :latestVideo.statistics.digg_count,
            sec_user_id: secUserId,
          };
        } else {
          this.logger.warn(`New video found but no URL for ${secUserId}: ${currentAwemeId}`);
          return null;
        }
      }

      return null; // No new video
    } catch (error) {
      this.logger.error(
        `Failed to fetch data for ${secUserId}: ${error.message}`,
      );
      // 可以在这里添加更详细的错误处理，例如重试机制
      throw error;
    }
  }
}
